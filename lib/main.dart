...
class _RaidScreenState extends State<RaidScreen> {
            ...
            return Stack(
              children: [
                CustomPaint(
                  ...
                  painter: FortressPainter(
                    progress: _progress,
                    ghost: _ghostProgress,
                  ),
                ),
                Padding(
    ...
    return Row(
      children: [
        ...
        _hudChip(
          'HEAT',
          _chase ? 'HIGH' : 'LOW',
          color: _chase ? const Color(0xFFED28C5) : const Color(0xFFA9FF2F),
        ),
      ],
    );
  }
  ...
  Widget _hudChip(
    String label,
    String value, {
    Color color = const Color(0xFF25E7FF),
  }) {
    return Container(
  ...
  Widget _buildChaseBanner() => Container(
    ...
    child: Row(
      children: [
        ...
        const Expanded(child: Text('DETECTED — choose an alternative route')),
        ElevatedButton(onPressed: _escape, child: const Text('ESCAPE')),
      ],
    ),
  );
  ...
  Widget _buildControls() {
    return Column(
      children: [
        Row(
          ...
          children: [
            ...
            _actionButton(
              ...
              'GRAFFITI',
              _activateGraffiti,
              accent: _graffitiReady
                  ? const Color(0xFFED28C5)
                  : const Color(0xFF5E6078),
            ),
            const SizedBox(width: 12),
            _actionButton(
              Icons.diamond,
        ...
        Text(
          'ROUTE ${(100 * _progress).round()}%  ·  GHOST ${(100 * _ghostProgress).round()}%',
          style: const TextStyle(letterSpacing: 2, color: Color(0xFFCBD1E8)),
        ),
      ],
    );
  }
  ...
  Widget _actionButton(
    ...
    String label,
    VoidCallback action, {
    Color accent = const Color(0xFFCBD1E8),
  }) {
    return SizedBox(
      width: 82,
      height: 70,
      child: OutlinedButton(
        ...
        child: Column(
          children: [
            Icon(icon, size: 27),
            Text(label, style: const TextStyle(fontSize: 9, letterSpacing: 1)),
          ],
        ),
      ),
    );
  ...
  Widget _buildResultPanel() => Container(
    ...
    child: Column(
      children: [
        ...
        const Text(
          'Ghost comparison unlocked · Base reward ready',
          style: TextStyle(color: Color(0xFFCBD1E8)),
        ),
...
class FortressPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final route = Path()
      ..moveTo(30, routeY)
      ..lineTo(size.width * .33, routeY - 75)
      ..lineTo(size.width * .58, routeY + 18)
      ..lineTo(size.width - 36, routeY - 105);
    canvas.drawPath(route, cyan);
    canvas.drawLine(
      Offset(size.width * .46, routeY - 10),
      Offset(size.width * .58, routeY + 18),
      magenta,
    );
    canvas.drawCircle(
      Offset(size.width * progress.clamp(.05, .95), routeY - 35),
      18,
      lime,
    );
    canvas.drawCircle(
      Offset(size.width * ghost.clamp(.05, .95), routeY - 70),
      15,
      Paint()..color = const Color(0xFFA78BFA),
    );
    final graffiti = Paint()
      ..color = const Color(0xFFED28C5)
      ..strokeWidth = 7
      ..style = PaintingStyle.stroke;
    canvas.drawArc(
      Rect.fromLTWH(size.width * .68, routeY - 180, 90, 90),
      .3,
      4.6,
      false,
      graffiti,
    ...
    canvas.drawLine(
      Offset(size.width * .73, routeY - 150),
      Offset(size.width * .82, routeY - 205),
      lime,
    );
  }

  @override
  bool shouldRepaint(covariant FortressPainter oldDelegate) =>
      oldDelegate.progress != progress || oldDelegate.ghost != ghost;
}
