...
The versioned `web/flutter_bootstrap.js` therefore calls the official Flutter loader with `forceSingleThreadedSkwasm: true`. This makes the current Pages behavior explicit and removes the misleading multithreading warning. It does not claim multithreaded rendering or equivalent performance.
